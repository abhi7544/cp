/*EXPLANATION
Since the matrices may not be invertible, we cannot store prefix products for range queries. Thus we need a data structure. We can use segment tree for range queries. Each input matrix is stored leaf nodes of the tree. Combining two nodes can be done like :-

$$Val_{node}$$ = $$Val_{node->left}Val_{node->right}$$ mod $$r$$. Where $$Val_{node}$$ is a 22 matrix.
